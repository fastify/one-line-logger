...
const messageFormatFactory = (levels, colors, useColors, options = {}) => {
  let customColors
  if (colors != null) {
  ...
  const messageFormat = (log, messageKey) => {
    const time = formatDate(log.time, options)
    const level = levelLookUp[log.level]
    const reqId = log.reqId ? `${log.reqId} - ` : ''

    return log.req
      ? `${time} - ${level} - ${reqId}${log.req.method} ${
          log.req.url
        } - ${colorizeMessage(log[messageKey])}`
      : `${time} - ${level} - ${reqId}${colorizeMessage(log[messageKey])}`
  }
