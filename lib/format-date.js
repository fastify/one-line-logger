...
function formatDate (timestamp, options = {}) {
  ...
  if (options.customTimeFormat) {
    return dateformat(date, options.customTimeFormat)
  }

  // Time-only format (no date, no timezone)
  if (options.timeOnly) {
    return TWO_PADDED_NUMBERS[date.getHours()] + ':' +
      TWO_PADDED_NUMBERS[date.getMinutes()] + ':' +
      TWO_PADDED_NUMBERS[date.getSeconds()] + '.' +
      THREE_PADDED_NUMBERS[date.getMilliseconds()]
  }

  // Default: full date with timezone
